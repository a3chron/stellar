...
#
# Why this exists: nix/package.nix carries three values that all have to move
# together on every release - the version, the SRI hash of the release tarball
# and the commit the ldflags stamp. They cannot be collapsed into one edit,
...
if [ "$DRY_RUN" = true ]; then
  ...
  echo "         then pin version/hash/commit in nix/package.nix and push that."
  exit 0
fi
...
sed -i \
  ...
  -e "s|^    \"-X main.commit=.*\"|    \"-X main.commit=$COMMIT\"|" \
  "$PACKAGE_NIX"
...
grep -q "main.commit=$COMMIT"     "$PACKAGE_NIX" || die "commit pin did not apply"
ok "version, hash and commit updated"
...
Updates version, source hash and stamped commit in nix/package.nix to
